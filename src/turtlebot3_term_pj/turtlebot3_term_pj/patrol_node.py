...
class PatrolNode(Node):
    def __init__(self):
        ...
        self.stop_patrol_service = self.create_service(
            StopPatrol, '/stop_patrol', self.stop_patrol_service_callback)
        
        # 타이머 설정
        self.timer = self.create_timer(0.1, self.timer_callback)

    def stop_patrol_service_callback(self, request, response):
        if self.is_patrol_active:
            self.is_patrol_active = False
            if not self.navigator.isTaskComplete():
                self.navigator.cancelTask()  # 진행 중인 내비게이션 작업 취소
            response.success = True
            response.message = 'Patrol stopped'
        else:
            response.success = False
            response.message = 'Patrol is not active'
        self.get_logger().info(f'Stop patrol service called: {response.message}')
        return response
