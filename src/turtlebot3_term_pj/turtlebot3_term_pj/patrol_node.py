...
class PatrolNode(Node):
    def __init__(self):
        ...
        self.is_processing_point = False  # 새로운 상태 변수 추가
        ...
        # 액션 클라이언트 (TurnToDesk)
        self.turn_client = ActionClient(self, BottleAlign, 'bottle_align')
    ...
    def stop_patrol_callback(self, msg):
        ...
        self.is_processing_point = False  # 처리 중 상태 해제
    ...
    def timer_callback(self):
        ...
        if self.navigator.isTaskComplete():
            ...
            if result == TaskResult.SUCCEEDED:
                ...
                self.is_processing_point = True  # 처리 중 상태로 설정
                self.turn_to_desk()
            else:
    ...
    def turn_to_desk(self):
        ...
        if not self.turn_client.wait_for_server(timeout_sec=2.0):
            ...
            self.is_processing_point = False  # 실패 시 상태 해제
            return

        goal_msg = BottleAlign.Goal()
        goal_msg.start = True
    ...
    def yolo_response_callback(self, future):
        ...
        try:
            ...
            if self.is_patrol_active:
                ...
                self.is_processing_point = False  # 처리 완료 후 상태 해제
                self.send_next_goal()
            else:
                self.get_logger().info('패트롤 중단됨')
                self.status_pub.publish(String(data='Patrol stopped'))
                self.is_processing_point = False  # 중단 시 상태 해제
        except Exception as e:
            self.get_logger().error(f'yolo_node 호출 실패: {str(e)}')
            self.is_processing_point = False  # 예외 발생 시 상태 해제
