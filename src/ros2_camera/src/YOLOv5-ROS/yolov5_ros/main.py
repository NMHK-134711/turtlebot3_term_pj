...
class yolov5_ros(Node):
    def __init__(self):
        ...
        self.get_logger().info('YOLO 노드가 시작되었습니다.')
        self.get_logger().info('토픽 구독 중:')
        self.get_logger().info('  - /image_raw: 이미지 수신')
        self.get_logger().info('  - /yolo_control: 제어 신호 수신')

        # parameter - 절대 경로로 수정
        self.declare_parameter('weights', '/home/kimtaehee/ros2_main/src/ros2_camera/src/YOLOv5-ROS/yolov5_ros/yolov5_ros/config/yolov5s.pt')
        self.declare_parameter('data', '/home/kimtaehee/ros2_main/src/ros2_camera/src/YOLOv5-ROS/yolov5_ros/yolov5_ros/data/coco128.yaml')
    ...
    def control_callback(self, msg):
        ...
        if msg.data == "start_detection":
            ...
            self.get_logger().info('YOLO 감지 활성화!')
            
            # 최신 이미지로 즉시 감지 실행
            if self.latest_image is not None:
                self.process_detection(self.latest_image)
                
        elif msg.data == "stop_detection":
            self.detection_active = False
            self.get_logger().info('YOLO 감지 비활성화!')
        else:
            self.get_logger().warn(f'알 수 없는 제어 신호: {msg.data}')
    ...
    def process_detection(self, image: Image):
        """실제 감지 처리 함수"""
        ...
        self.pub_image.publish(self.bridge.cv2_to_imgmsg(im0, encoding='bgr8'))
    ...
    def image_callback(self, image: Image):
        """이미지 콜백 - 이미지는 계속 받지만 감지는 제어됨"""
        # 최신 이미지 저장 (제어 신호가 올 때 사용)
        self.latest_image = image
        
        # 감지가 활성화된 경우에만 처리
        if self.detection_active:
            self.process_detection(image)
